...
-- Run this block after creating the guest user via Supabase Auth
-- Email: invitado@applimpieza.demo
-- =============================================================================
DO $$
DECLARE
    guest_user_id UUID;
...
BEGIN
    ...
    FROM auth.users
    WHERE email = 'invitado@applimpieza.demo';

    IF guest_user_id IS NULL THEN
        RAISE NOTICE 'Guest user not found. Please create it via Supabase Auth first.';
        RETURN;
    END IF;

    -- Guard: skip if data already seeded
    IF EXISTS (SELECT 1 FROM clientes WHERE user_id = guest_user_id LIMIT 1) THEN
        RAISE NOTICE 'Guest data already seeded. Skipping.';
        RETURN;
    END IF;
    ...
    RAISE NOTICE 'Created 3 clients for guest';
    ...
    RAISE NOTICE 'Created 2 invoices with lines for guest';
    ...
    RAISE NOTICE 'Created 5 expenses for guest';

    RAISE NOTICE 'Guest seed data inserted successfully for user: %', guest_user_id;
